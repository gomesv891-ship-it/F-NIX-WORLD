...
interface DocumentoEditorModalProps {
  ...
  onPreencherModelo?: (mod: DocumentoItem) => void;
}
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-[#070d19] text-slate-100 select-none overflow-hidden font-sans">
      ...
      <div className="h-14 px-4 bg-[#0a1222] border-b border-slate-800 flex items-center justify-between shrink-0 z-30 shadow-md">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Opção para Preencher Documento quando for Modelo */}
          {documento?.isModelo && onPreencherModelo && (
            <button
              type="button"
              onClick={() => onPreencherModelo(documento)}
              className="px-3.5 py-1.5 text-xs font-bold text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 rounded-lg shadow-md transition-all flex items-center gap-1.5 cursor-pointer"
              title="Abrir tela dividida para preencher dados e gerar um novo documento a partir deste modelo"
            >
              <Play className="w-3.5 h-3.5 fill-white" />
              <span className="hidden sm:inline">Preencher Documento</span>
            </button>
          )}
        </div>
      </div>
