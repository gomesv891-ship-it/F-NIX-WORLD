...
export const ConfiguracoesScreen: React.FC<ConfiguracoesScreenProps> = ({
  ...
  const notifSoundCategories: {
    ...
    },
    {
      id: 'pendencias',
      label: 'Pendências',
      description: 'Toque para novas pendências criadas, atribuídas a você ou com alteração de status na aba Pendências',
      icon: AlertCircle,
    },
    {
      id: 'boletos',
    ...
    {
      id: 'tabelas',
      label: 'Tabela Comercial / Revenda',
      description: 'Toque para criação, edição, sincronização de preços e atualizações da Tabela de Revenda',
      icon: Table,
    },
    {
      id: 'documentos',
      label: 'Documentos e Modelos Editáveis',
      description: 'Avisos e alertas de novos documentos salvos, catálogos e modelos editáveis',
      icon: FolderKanban,
    },
    {
      id: 'outras',
      label: 'Outras notificações do sistema',
      description: 'Avisos de notas, comunicados gerais e outros alertas do CRM',
      icon: Bell,
    },
  ];
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-xs space-y-8 animate-in fade-in duration-200 max-w-4xl">
          ...
          <div className="space-y-4">
            ...
            <div className="divide-y divide-slate-100 border border-slate-200/80 rounded-2xl p-2 bg-white">
              ...
              {/* Gatilho 4: Alerta de Pendências da Aba Pendências */}
              <div className="py-3 px-3 flex items-center justify-between gap-4">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5">
                    <h4 className="text-xs sm:text-sm font-bold text-slate-800">
                      Alertas de Pendências (Aba Pendências)
                    </h4>
                    <span className="px-1.5 py-0.5 rounded-md bg-blue-50 border border-blue-200 text-[#1D4ED8] text-[10px] font-bold">
                      Sininho + Som
                    </span>
                  </div>
                  <p className="text-[11px] sm:text-xs text-slate-500">
                    Notificar no sininho e emitir o toque sonoro configurado quando uma pendência for criada, atribuída a você ou atualizada.
                  </p>
                </div>
                <button
                  type="button"
              ...
              {/* Gatilho 7: Alertas da Tabela Comercial / Revenda */}
              <div className="py-3 px-3 flex items-center justify-between gap-4">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-blue-600 inline-block"></span>
                    <h4 className="text-xs sm:text-sm font-bold text-slate-800">
                      Alertas da Tabela de Revenda
                    </h4>
                  </div>
                  <p className="text-[11px] sm:text-xs text-slate-500">
                    Disparar aviso no sininho (com som) ao atualizar, sincronizar preços do CRM ou salvar a Tabela de Revenda.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() =>
                    setNotifPrefs((prev) => ({
                      ...prev,
                      alerts: { ...prev.alerts, tabelas: !(prev.alerts?.tabelas ?? true) },
                    }))
                  }
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    notifPrefs.alerts?.tabelas !== false ? 'bg-[#1D4ED8]' : 'bg-slate-200'
                  }`}
                  role="switch"
                  aria-checked={notifPrefs.alerts?.tabelas !== false}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition duration-200 ease-in-out ${
                      notifPrefs.alerts?.tabelas !== false ? 'translate-x-5' : 'translate-x-0'
                    }`}
                  />
                </button>
              </div>

              {/* Gatilho 8: Alertas de Documentos e Modelos Editáveis */}
              <div className="py-3 px-3 flex items-center justify-between gap-4">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-teal-500 inline-block"></span>
                    <h4 className="text-xs sm:text-sm font-bold text-slate-800">
                      Alertas de Documentos e Modelos Editáveis
                    </h4>
                  </div>
                  <p className="text-[11px] sm:text-xs text-slate-500">
                    Notificar no sininho (com som) ao salvar novos documentos, catálogos e modelos editáveis no sistema.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() =>
                    setNotifPrefs((prev) => ({
                      ...prev,
                      alerts: { ...prev.alerts, documentos: !(prev.alerts?.documentos ?? true) },
                    }))
                  }
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    notifPrefs.alerts?.documentos !== false ? 'bg-[#1D4ED8]' : 'bg-slate-200'
                  }`}
                  role="switch"
                  aria-checked={notifPrefs.alerts?.documentos !== false}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition duration-200 ease-in-out ${
                      notifPrefs.alerts?.documentos !== false ? 'translate-x-5' : 'translate-x-0'
                    }`}
                  />
                </button>
              </div>
