...
import {
  ...
  fetchDocumentosEnviosFromDatabase,
  getCatalogos,
  saveCatalogoAsync,
  updateCatalogoAsync,
  deleteCatalogoAsync,
...
import { ImportarArquivoModeloModal } from './documentos/ImportarArquivoModeloModal';
import { TabelaComercialEditorModal } from './documentos/TabelaComercialEditorModal';
import { SupabaseStorageConfigModal } from './documentos/SupabaseStorageConfigModal';
...
export const DocumentosScreen: React.FC<DocumentosScreenProps> = ({
  ...
  // Modal de Confirmação de Exclusão (Prompt 5)
  ...
  // Categorias válidas e estáveis por aba (100% dinâmicas e independentes do Supabase)
  const catalogoCategoriasMemo = useMemo(() => {
    return categorias.filter((c) => c.tipo === 'catalogo');
  }, [categorias]);

  const documentoCategoriasMemo = useMemo(() => {
    return categorias.filter((c) => c.tipo === 'documento');
  }, [categorias]);

  const modeloCategoriasMemo = useMemo(() => {
    return categorias.filter((c) => c.tipo === 'modelo');
  }, [categorias]);
  ...
  const handleDelete = (
    ...
    const isModelo = 'isModelo' in item && Boolean(item.isModelo);
    const isTabela =
      ('isTabelaComercial' in item && Boolean(item.isTabelaComercial)) ||
      (isModelo && item.titulo.toLowerCase().includes('tabela'));
    const tipoLabel = isTabela
      ? 'Tabela Comercial / Modelo Editável'
      : isModelo
      ? 'Modelo Editável'
      : tipo === 'catalogo'
      ? 'Catálogo'
    ...
    setItemToDelete({ item, tipo, tipoLabel });
  };

  const handleConfirmDeleteItem = async () => {
    if (!itemToDelete) return;
    const { item, tipo } = itemToDelete;
    setIsDeletingItem(true);

    // 1. Remove imediatamente da tela para feedback instantâneo
    if (tipo === 'catalogo') {
      setCatalogos((prev) => prev.filter((c) => c.id !== item.id));
    } else {
      setDocumentos((prev) => prev.filter((d) => d.id !== item.id));
    }

    try {
      // 2. Exclui do Supabase Database + Storage + IndexedDB e grava exclusão permanente
      if (tipo === 'catalogo') {
        await deleteCatalogoAsync(item.id, currentUserName);
      } else {
        await deleteDocumentoAsync(item.id, currentUserName);
      }
      showToast(`"${item.titulo}" excluído permanentemente.`);
      await reloadData();
    } catch (err) {
      console.error('Erro ao excluir item do Supabase:', err);
      showToast(`Erro ao excluir "${item.titulo}". Tente novamente.`);
    } finally {
  ...
  const handleCopyWhatsAppLink = async (item: CatalogoItem) => {
    const res = await copyCatalogoWhatsAppLink(item);
    if (res.success) {
      if (res.isStorageConfigPending) {
        showToast('Link copiado! Dica: Execute o script SQL no Supabase para link público via CDN.');
      } else {
        showToast('Link do catálogo copiado para o WhatsApp!');
      }
    } else {
      showToast(`Não foi possível copiar: ${res.error || 'Erro desconhecido'}`);
    }
  };
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-14 py-6 sm:py-8 space-y-6 flex-1 flex flex-col font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-1">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-white hover:bg-slate-50 text-slate-700 text-xs sm:text-sm font-semibold border border-slate-200/90 shadow-2xs transition-all cursor-pointer"
            title="Script SQL para criar buckets e regras RLS no Supabase Storage"
          >
            <Database className="w-4 h-4 text-[#0052cc]" />
            <span>Storage Supabase (SQL)</span>
          </button>
          ...
          {activeArea === 'modelos' && (
            <div className="flex flex-wrap items-center gap-2">
              <button
                onClick={() => {
                  const termo =
                    documentos.find(
                      (d) =>
                        d.id === 'termo_autorizacao_pagamento_link' ||
                        (d.titulo.toLowerCase().includes('autorização') && d.isModelo)
                    ) || getInitialTermoAutorizacaoPagamentoDoc();
                  setModeloParaUsar(termo);
                  setIsUsarModeloOpen(true);
                }}
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-[#0052cc] text-xs sm:text-sm font-bold border border-blue-200 shadow-2xs transition-all cursor-pointer"
                title="Abrir Preenchimento do Termo de Autorização de Pagamento por Link"
              >
                <CreditCard className="w-4 h-4 text-[#0052cc]" />
                <span>Termo de Pagamento por Link</span>
              </button>
              ...
              <button
                ...
                title="Importar arquivo (PDF, Word, Excel, PowerPoint, ODT, SVG, HTML) como modelo editável"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredCatalogos.map((cat) => (
                <ItemCard
                  ...
                  onCopyWhatsApp={() => handleCopyWhatsAppLink(cat)}
                  onShare={() => {
                    setViewerItem(cat);
                    setIsViewerOpen(true);
                  }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {allFavoritos.map((fav) => (
                <ItemCard
                  ...
                  onCopyWhatsApp={
                    fav._origem === 'catalogo'
                      ? () => handleCopyWhatsAppLink(fav as CatalogoItem)
                      : undefined
                  }
                  onShare={() => {
                    setViewerItem(fav);
            ...
            if (res.success) {
              ...
              try {
                sendUserNotification({
                  category: 'Documentos',
                  title: `Catálogo Salvo: ${res.item?.titulo || data.titulo}`,
                  description: `Catálogo cadastrado com sucesso na categoria ${res.item?.categoriaNome || 'Geral'}.`,
          ...
          onSave={async (data, file, onProg) => {
            ...
            if (res.success) {
              ...
              try {
                sendUserNotification({
                  category: 'Documentos',
                  title: `Documento Salvo: ${res.item?.titulo || data.titulo}`,
                  description: `Documento registrado com sucesso na categoria ${res.item?.categoriaNome || 'Geral'}.`,
                  targetTab: 'Documentos',
                  authorName: currentUserName,
                });
              } catch {}
      ...
      {isEditorOpen && editorDoc && (
        <DocumentoEditorModal
          isOpen={isEditorOpen}
          documento={editorDoc}
          categorias={editorDoc.isModelo ? modeloCategoriasMemo : documentoCategoriasMemo}
          clients={clients}
          currentUserName={currentUserName}
          onClose={() => {
        ...
        <AlterarCategoriaModal
          ...
          categorias={
            alterarCatTarget.tipo === 'catalogo'
              ? catalogoCategoriasMemo
              : ('isModelo' in alterarCatTarget.item && Boolean((alterarCatTarget.item as any).isModelo))
              ? modeloCategoriasMemo
              : documentoCategoriasMemo
          }
      ...
      {/* MODAL: Importar Arquivo como Modelo */}
      {isImportarPdfModeloOpen && (
        <ImportarArquivoModeloModal
          isOpen={isImportarPdfModeloOpen}
          categorias={modeloCategoriasMemo}
          currentUserName={currentUserName}
          ...
          onSuccess={(saved) => {
            ...
            setIsModeloEditorOpen(true);
            showToast(`Arquivo "${saved.titulo}" importado como modelo editável no Supabase!`);
          }}
        />
      )}
      ...
      {/* MODAL: Confirmação de Exclusão Definitiva (Prompt 5) */}
      {itemToDelete && (
        <div
        ...
        >
          <div
            className="w-full max-w-md bg-white border border-rose-200 rounded-2xl shadow-2xl p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150 text-slate-900"
          ...
          >
            <div className="flex items-start gap-3.5">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">
                  Excluir {itemToDelete.tipoLabel}?
                </h3>
                <p className="text-xs text-rose-600 font-medium mt-0.5">
                  Esta ação excluirá o registro e os arquivos do Supabase
                </p>
              </div>
            </div>
            ...
            <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1.5 text-xs text-slate-600">
              ...
              <p className="font-bold text-slate-900 text-sm break-words">
                "{itemToDelete.item.titulo}"
              </p>
              <p className="text-[11px] text-slate-500 pt-1 border-t border-slate-200/60">
                O registro será removido do banco de dados do Supabase e quaisquer arquivos associados serão deletados do storage. Esta operação não poderá ser desfeita.
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              ...
              <button
                ...
                disabled={isDeletingItem}
                className="px-5 py-2 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 disabled:opacity-50 rounded-xl flex items-center gap-2 cursor-pointer shadow-sm shadow-rose-600/25 transition-all"
              >
                <Trash2 className="w-4 h-4" />
                <span>{isDeletingItem ? 'Excluindo...' : 'Excluir Permanentemente'}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* MODAL: Configuração do Supabase Storage (SQL & Verificação) */}
  ...
  return (
    <div className="bg-white hover:bg-slate-50/50 border border-slate-200/90 hover:border-slate-300 rounded-2xl p-4 transition-all duration-150 flex flex-col justify-between group shadow-[0_2px_12px_-2px_rgba(0,0,0,0.03)] hover:shadow-md">
      <div>
        ...
        {/* As duas formas de trabalhar com o Modelo: 1. Preencher Documento (tela dividida) | 2. Editar o Modelo (elementos) */}
        {onUseModelo && !('isTabelaComercial' in item && item.isTabelaComercial) && (
          <div className="grid grid-cols-2 gap-2 mb-3">
            <button
              ...
              className="py-2 px-2 bg-[#0052cc] hover:bg-[#0047b3] text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm shadow-blue-600/25 transition-all cursor-pointer group/btn"
              title="Preencher Documento: abre tela dividida com formulário à esquerda e prévia em tempo real à direita"
            >
            ...
            <button
              ...
              className="py-2 px-2 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer group/btn"
              title="Editar o Modelo: continuar podendo editar livremente todos os elementos do documento"
            >
              <Pencil className="w-3.5 h-3.5 text-slate-600 group-hover/btn:scale-110 transition-transform" />
              <span>Editar Modelo</span>
            </button>
          </div>
        )}

        {/* Botão de Destaque "Copiar para WhatsApp" para Catálogos */}
        {onCopyWhatsApp && (
          <button
            type="button"
            onClick={(e) => {
            ...
            className="w-full mb-3 py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm shadow-emerald-600/25 transition-all cursor-pointer group/btn"
            title="Copiar link real do catálogo salvo no Supabase para WhatsApp"
          >
            <MessageCircle className="w-3.5 h-3.5 fill-white group-hover/btn:scale-110 transition-transform" />
        ...
        <div className="flex items-center gap-1">
          ...
          {onCopyWhatsApp && (
            <button
              ...
              title="Copiar link do catálogo para WhatsApp"
            >
        ...
        <div className="relative">
          ...
          {isMenuOpen && (
            <div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              {/* Opção Primária se for Catálogo: Copiar para WhatsApp */}
              {onCopyWhatsApp && (
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="w-full text-left px-2.5 py-1.5 rounded-lg text-emerald-700 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 flex items-center gap-2 transition-colors cursor-pointer font-bold border-b border-slate-100 mb-1"
                >
              ...
              {/* Opção Primária se for Modelo: Preencher Documento */}
              {onUseModelo && (
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="w-full text-left px-2.5 py-2 rounded-lg text-blue-700 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 flex items-center gap-2 transition-colors cursor-pointer font-bold border-b border-slate-100 mb-1"
                >
                  <Play className="w-3.5 h-3.5 fill-current text-[#0052cc]" />
                  <span>Preencher Documento</span>
                </button>
              )}

              {/* Editar o Modelo Livremente */}
              <button
                type="button"
                onClick={() => {
                  setActiveMenuId(null);
                ...
                className="w-full text-left px-2.5 py-1.5 rounded-lg text-slate-700 hover:text-slate-900 hover:bg-slate-100 flex items-center gap-2 transition-colors cursor-pointer font-medium"
              >
                <Pencil className="w-3.5 h-3.5 text-blue-600" />
                <span>{origemBadge === 'Modelo' ? 'Editar Modelo (Livremente)' : 'Editar'}</span>
              </button>

              {/* Configurar Campos do Formulário */}
              {origemBadge === 'Modelo' && onEditCampos && (
                <button
                  type="button"
                  onClick={() => {
                    setActiveMenuId(null);
                ...
                >
                  ...
                  <span>Configurar Campos do Formulário</span>
                </button>
              )}
