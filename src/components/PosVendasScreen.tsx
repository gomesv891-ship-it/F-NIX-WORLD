...
import { saveItemToSupabase, deleteItemFromSupabase, saveWholeCollectionToSupabase } from '../utils/supabaseClient';
...
export const PosVendasScreen: React.FC<PosVendasScreenProps> = ({
  ...
  const handleStatusChange = (id: string, newStatus: PosVendaStatus, e?: React.MouseEvent) => {
    ...
    setRecords((prev) => {
      ...
      if (updatedItem) {
        saveItemToSupabase('fenix_pos_vendas_db', { ...updatedItem, status: newStatus, updatedAt: new Date().toISOString() }, 'id', currentUserName).catch(() => {});
      }
      try {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setRecords(updated);
      localStorage.setItem('fenix_pos_vendas_db', JSON.stringify(updated));
      await deleteItemFromSupabase('fenix_pos_vendas_db', recordToDelete.id, 'id', currentUserName);
