...
export const DEFAULT_SOUNDS_PER_TYPE: Record<NotificationCategoryType, NotificationSoundType> = {
  ...
  documentos: 'Chime Cristalino com Ressonância',
...
export function normalizeCategoryKey(category: string): NotificationCategoryType {
  ...
  if (c === 'documentos' || c === 'documento' || c === 'catálogos' || c === 'catalogos' || c === 'catalogo') {
    return 'documentos';
  }
  return 'outras';
}
...
export function playUserCustomSoundForCategory(
  ...
  // Volume seguro (se for 0 ou inválido, usa 85% como padrão para garantir que alertas toquem)
  const volume = typeof prefs.volumePercent === 'number' && prefs.volumePercent > 0
    ? prefs.volumePercent
    : 85;
  ...
  // 2. Se a categoria for puramente secundária (apenas 'notas') e não tiver arquivo próprio,
  ...
  if (
    ...
    normalizedCat !== 'chat' &&
    normalizedCat !== 'pendencias' &&
    normalizedCat !== 'tabelas' &&
    normalizedCat !== 'documentos'
  ) {
    ...
    if (customOutras?.dataUrl) {
      ...
      playNotificationAudioSnippet(customOutras.dataUrl, startTime, duration, volume);
      return;
    }
  }
...
export function isAlertCategoryEnabled(
  userId: string,
  category: 'pendencias' | 'notas' | 'tarefas' | 'followup' | 'boletos' | 'metas' | 'chat' | 'estoque' | 'tabelas' | 'documentos'
): boolean {
  const prefs = getUserNotificationPreferences(userId);
