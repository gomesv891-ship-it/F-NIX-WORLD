...
export async function registrarEntradaEstoque(params: {
  ...
  try {
    ...
    const saveMovs = await saveWholeCollectionToSupabase(ESTOQUE_MOVIMENTACOES_KEY, updatedMovs, params.usuario);
    if (!saveMovs.success) return { success: false, error: saveMovs.error };
...
export async function registrarSaidaVendaEstoque(params: {
  ...
  try {
    ...
    const saveMovsRes = await saveWholeCollectionToSupabase(ESTOQUE_MOVIMENTACOES_KEY, updatedMovs, params.usuario);
    if (!saveMovsRes.success) return { success: false, error: saveMovsRes.error };
