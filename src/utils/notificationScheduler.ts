...
export function checkFollowUpTwoDayAlerts(): void {
  ...
  try {
    const now = new Date();
    // Requisito estrito: Notificar no sininho a partir das 08:00
    if (now.getHours() < 8) {
      return;
    }
    ...
    const updatedItems = items.map((item) => {
      ...
      // Regra explícita: Se o Follow-up estiver marcado como Vendido ou Perdido:
      // Encerrar os lembretes automaticamente.
      ...
      // Regra: Permitir desativar pelo próprio sininho ou no Follow-up (lembretesAtivos === false)
      if (item.lembretesAtivos === false) {
        return item;
      }

      // 1. Não repetir durante o mesmo dia
      const lastAlertDate = localStorage.getItem(alertKey);
      if (lastAlertDate === todayDateStr) {
        return item;
      }

      // 2. Se continuar pendente: Notificar novamente a cada 2 dias, sempre às 08:00
      ...
      if (shouldNotify) {
        ...
        sendUserNotification({
          ...
          description: `Orçamento #${item.pedido || item.id} de ${item.cliente} (${item.produto || 'Orçamento'} • ${valorFormatado}) pendente para novo contato.`,
          targetTab: 'Follow-up',
